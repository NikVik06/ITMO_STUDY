...
import project.converters.CarConverter;
...
public class HumanBeing implements Comparable<HumanBeing>, Validatable {
    ...
    public HumanBeing(String name, Coordinates coordinates, ZonedDateTime creationDate, Boolean realHero, boolean hasToothpick,
     ...
     */
    public static void updateNextId(CollectionManager manager) {
        int maxId = manager.getCollection().stream()
                .filter(Objects::nonNull)
                .mapToInt(HumanBeing::getId)
                .max()
                .orElse(0);
        idCounter.set(maxId + 1);  // Атомарная установка
    }
